...
import { ArrowLeft, Mail, Lock, ChevronRight, Eye, EyeOff } from 'lucide-react';
...
const Login: React.FC<NavigationProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('unipost_credentials');
    if (saved) {
      try {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (rememberMe) {
      localStorage.setItem('unipost_credentials', JSON.stringify({ email, password }));
    } else {
      localStorage.removeItem('unipost_credentials');
    }
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col justify-center py-8 md:py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="mx-auto w-full max-w-[440px]">
        ...
        <div className="flex justify-center items-center gap-3 mb-6 md:mb-8">
          ...
          <span className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tighter text-center">UniPost</span>
        </div>
      </div>
        ...
        <div className="bg-white py-8 md:py-10 px-6 md:px-10 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.08)] rounded-[40px] border border-gray-100/60">
          ...
          <div className="relative mb-8">
            ...
            <div className="relative flex justify-center text-[10px] uppercase">
              <span className="bg-white px-4 text-gray-400 font-bold tracking-widest">ou continue com e-mail</span>
            </div>
          </div>
          ...
          <form className="space-y-5" onSubmit={handleLogin}>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button 
                  ...
                  className="text-[10px] font-bold text-brand hover:text-brand-700 transition-colors uppercase tracking-widest"
                >
                  Esqueceu a senha?
                </button>
              </div>
            ...
            <div className="flex items-center pt-1">
              ...
              />
              <label htmlFor="remember-me" className="ml-3 block text-sm text-gray-500 font-medium cursor-pointer select-none">
                Lembrar de mim
              </label>
            </div>
